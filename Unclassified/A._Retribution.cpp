...
double sumMinDistance(vector<pair<int, int>> &pers, int &n, vector<pair<int, int>> &obj, int &k){
    double sum = 0;
    for(int i = 0; i < n; i++){
	double min_Distance = infinity;
	for(int j = 0; j < k ; j++){
	    double distance = sqrt(pow2(pers[i].first - obj[j].first) + sqrt(pow2(pers[i].second - obj[j].second)));
